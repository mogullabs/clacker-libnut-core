...
try {
  const permissions = require("@mogullabs/clacker-node-mac-permissions");
  ...
  const askForAccessibility = (nativeFunction, functionName) => {
    ...
    if (accessibilityStatus === "authorized") {
      ...
      return wrapWithWarning(
        `##### WARNING! The application running this script tries to access accessibility features to execute ${functionName}! Please grant requested access and visit https://github.com/mogullabs/clacker-nut.js#macos for further information. #####`,
        nativeFunction
      );
    }
  };
  const askForScreenRecording = (nativeFunction, functionName) => {
    ...
    if (screenCaptureStatus === "authorized") {
      ...
      return wrapWithWarning(
        `##### WARNING! The application running this script tries to screen recording features to execute ${functionName}! Please grant the requested access and visit https://github.com/mogullabs/clacker-nut.js#macos for further information. #####`,
        nativeFunction
      );
    }
  };
